...
@Component({
  ...
  imports: [DatePipe],
  templateUrl: './baseball.html',
  styleUrl: './baseball.css',
})
export class Baseball implements OnInit {
  readonly pageTitle = 'Baseball Odds';
  readonly subtitle = 'MLB odds from RapidAPI.';
  ...
  ngOnInit(): void {
    this.oddsService.getOddsBySport(SPORT_KEYS.baseball).subscribe({
      next: (events) => {
      ...
      error: (err) => {
        this.error = err?.message ?? 'Failed to load baseball odds.';
        this.loading = false;
      },
    });
  }
